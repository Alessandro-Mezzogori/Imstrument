...
public class Recording {
    ...
    public void record(short[] input) {
        ...
        */
        SampleInputStream sampleAudio= new SampleInputStream();
        AudioSystem.write(sampleAudio, AudioFileFormat.Type.WAVE, newRecord);

    }

    public void startRecording() {
        File newRecord = new File(StartApp.defaultFolder.toString() + "/recordings/" + "NewRecords_" + java.time.LocalTime.now() + ".wav");
        isRecording = true;
    }
        ...
        public int read() throws IOException {
            while (available() > 0)
            {
                ...
                if(idx == 0)
                {
                    ...
                    readSample[0]=(char) ((int)(sample[cursor++]*Short.MAX_VALUE) >> 8);
